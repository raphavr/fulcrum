...
- **Hooks handle emission** - guaranteed to fire, responsible for HTTP POST
...
## Decision 8: Spec-Driven Development as Alternative Entry Point

### Context
Some tasks are well-defined and benefit from upfront specification. Others are exploratory and need brainstorming. Forcing one entry point frustrates engineers.

### Decision
**Add `/spec` command as an alternative to `/brainstorm`, both feeding into the same implementation pipeline.**

### Rationale

**Borrowed from Context Engineering Kit:**
- SDD (Spec-Driven Development) pattern is proven for well-defined tasks
- Upfront codebase analysis reduces incorrect assumptions
- Structured specs make requirements explicit and reviewable

**Adapted for Superpowers:**
- `/spec` is one entry point, `/brainstorm` is another, both optional
- Spec phase generates a structured document in `docs/specs/`
- Spec feeds into Superpowers' plan phase (same as brainstorm does)
- Engineer chooses the right entry point for their task

**Value Proposition:**
- Well-defined tasks → `/spec` → faster to correct specification
- Exploratory tasks → `/brainstorm` → discover the right approach first
- Complex tasks → `/brainstorm` first, then `/spec` for formal requirements
- Engineer maintains control over which path makes sense
