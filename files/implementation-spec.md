...
1. **Spec-Driven Development Skill + `/spec` Command** — Alternative entry point for well-defined tasks
...
│   ├── brainstorming/                    # [UPSTREAM] Original Superpowers skill
│   │   └── SKILL.md
...
│   │   └── SKILL.md
│   │
│   ├── spec-driven-development/          # [NEW] SDD workflow (/spec entry point)
│   │   └── SKILL.md
...
│   ├── brainstorm.md                     # [UPSTREAM] /brainstorm
│   ├── write-plan.md                     # [UPSTREAM] /write-plan
│   ├── execute-plan.md                   # [UPSTREAM] /execute-plan
│   ├── spec.md                           # [NEW] /spec — SDD entry point
│   └── compound.md                       # [NEW] /compound — trigger learning capture
│
├── hooks/
│   ├── hooks.json                        # Hook definitions (SessionStart + Stop)
│   ├── metrics-session-start.js          # [NEW] Session start metrics collection
│   └── metrics-session-end.js            # [NEW] Session end metrics collection + emission
│
├── config/
...
#### Purpose
Provide an alternative entry point to the Superpowers workflow that transforms a prompt into a structured specification through iterative refinement, then feeds that spec into Superpowers' existing plan → implement pipeline.

#### Triggering Conditions
- User invokes `/spec` command
- User says "create a spec for", "write a spec", "I need a specification"

#### Behavior

The /spec flow has three phases that map onto Superpowers' existing pipeline:
...
1. Identify the most recently completed cycle:
   ...
   - Find plan document (if brainstorm or plan was used)
...
// TODO v2: HTTP POST workflow.session.started event
// fetch(process.env.FULCRUM_METRICS_URL, { ... })
...
// TODO v2: HTTP POST event to FULCRUM_METRICS_URL
// fetch(process.env.FULCRUM_METRICS_URL, {
//   method: 'POST',
//   headers: { 'Content-Type': 'application/json' },
//   body: JSON.stringify(event)
// }).catch(err => console.error('[metrics] emission failed:', err.message));

console.log('[metrics] Session completed:', JSON.stringify(event, null, 2));
```
...
**Example (in brainstorming skill):**

At the start:
```
Append to .claude/workflow-state.jsonl: {"stage":"brainstorm","action":"entered","timestamp":"<ISO8601>"}
```

When approved:
```
Append to .claude/workflow-state.jsonl: {"stage":"brainstorm","action":"completed","timestamp":"<ISO8601>"}
```

**Same pattern for:** `spec`, `plan`, `implement`, `review`, `finish`.
...
**Emitted by:** SessionStart hook (v2)
...
**Emitted by:** SessionStop hook (v1 logs to console, v2 POSTs to backend)
...
- `entry_point`: First workflow stage entered (`brainstorm`, `spec`, `plan`, `unknown`)
...
Invokes the spec-driven development skill.

- If a description is provided: uses it directly as input
...
/spec Add user notification preferences
  ↓
Generate task spec → present for review → iterate → approve
  ↓
